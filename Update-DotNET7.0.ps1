<#
	Installs dot net packages if chocolatey is installed
#>
...
#configure chocolatey packages to be installed
$filter = "dotnet-7"


#determine if package installed
$packages = @((choco list --localonly | Where-Object{$_ -match $filter}).split(" ")| Where-Object{$_ -match $filter})
$packages = $packages |sort |get-unique
#install or upgrade all the selected packages
if ($packages.count -gt 0){
	$packages += "dotnet-7.0-windowshosting"
	$packages += "dotnet-7.0"
	$packages = $packages |sort |get-unique
	#install or upgrade all the selected packages
...
$packages = @("dotnet-7.0-windowshosting","dotnet-7.0")
foreach($package in $packages){
	#install
