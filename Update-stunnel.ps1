<#
  Installs or updates chocolatey
	Uses chocolatey to install or update Visual Studio Code package
#>
...
$packages = @("stunnel","maven")

#check if packages are installed
$installedpackages = @()
foreach($package in $packages){
	if(@(choco list --localonly) -match "package"){$installedpackages += $package}
}
